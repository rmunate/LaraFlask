...
class File:
    ...
    def delete(name):
        ...
        """
        try:
            file = os.path.join(Paths.files(), name)
            ...
            raise ValueError(f"[File]: Error al eliminar el archivo '{name}', {e}")
    ...
    def put_base64(name, content):
        ...
        try:
            ...
            file_path = os.path.join(Paths.files(), name)
            ...
            raise ValueError(f"[File]: Error Al Guardar Desde Base64, {e}")
    ...
    def list(subdirectory:str = None, full_path = False):
        ...
        if subdirectory is not None:
            list = os.listdir(File.path(subdirectory))
        else:
            list = os.listdir(Paths.files())

        files = []

        for file in list:
            fullpath = File.path(name=file)
    ...
    def get_content(name:str):
        ...
        try:
            ...
            raise ValueError(f"[File]: Error al leer el archivo '{name}', {e}")
